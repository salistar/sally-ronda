 ...
 * @description Leaderboard Ronda avec 3 portées : Mondial / Pays / Ville.
 * Données réelles depuis /leaderboards/ronda?scope=<...>, auto-détection
...
export default function LeaderboardScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      log.bin(`GET /leaderboards/ronda`, { scope, filter });
      const r = await api.getLeaderboardScoped('ronda', filter, scope, 50);
      log.bout('200 /leaderboards/ronda', `${r.entries.length} entrées`);

/**
 * @file (tabs)/index.tsx
 * @description Home Ronda. 4 actions principales : Créer, Rejoindre, vs Bot
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUser(u);
      log.bin('GET /challenges/daily/ronda');
      const ch = await api.getDailyChallenge('ronda');
      if (ch) {
        log.bout('200 /challenges/daily/ronda', { title: ch.title, reward: ch.rewardCoins });
        setChallenge(ch);
      }
  ...
  const handleChallenge = async () => {
    log.bin('POST /challenges/daily/ronda/matchmake');
    try {
      const room = await api.joinDailyChallenge('ronda');

 ...
 * @description Profil Ronda avec vraies valeurs (stats/coins/achievements/
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) /leaderboards/ronda/my-rank : best-effort
      try {
        log.bin('GET /leaderboards/ronda/my-rank');
        const r = await api.getMyRank('ronda', 'season');
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang ronda pour ce joueur');
